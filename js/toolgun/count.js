//Tool ID
cvar.set("toolgun_tool", "11");

//Tool text
cvar.set("toolgun_tooltext", "Count");
cvar.set("toolgun_tooltip1", "Set count of item");
cvar.set("toolgun_tooltip2", "Enter count");
cvar.set("toolgun_tooltip3", "in the spawn menu");
cvar.set("toolgun_tooltip4", " ");

//Tool mode name
cvar.set("toolgun_toolmode1", " ");
cvar.set("toolgun_toolmode2", " ");
cvar.set("toolgun_toolmode3", " ");
cvar.set("toolgun_toolmode4", " ");

//Tool properties name
cvar.set("toolgun_toolset1", "Count:");
cvar.set("toolgun_toolset2", "--------:");
cvar.set("toolgun_toolset3", "--------:");
cvar.set("toolgun_toolset4", "--------:");

//Tool default setting
cvar.set("uis_tg1", "10");
cvar.set("uis_tg2", "0");
cvar.set("uis_tg3", "0");
cvar.set("uis_tg4", "0");

//Tool modes toggle
cvar.set("uitoolmode", "toggle toolgun_mod5 0");