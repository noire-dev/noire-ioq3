...
bool CG_Asset_Parse(int handle) {
	...
	if(!trap_PC_ReadToken(handle, &token)) return false;
...
static bool CG_OwnerDrawHandleKey(int ownerDraw, int flags, float* special, int key) { return false; }
...
static int CG_PlayCinematic(const char* name, float x, float y, float w, float h) { return trap_CIN_PlayCinematic(name, x, y, w, h, CIN_loop); }
