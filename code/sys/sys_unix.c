...
static char* Sys_LegacyHomePath(void) {
	...
	if(!*homePath) {
		if((p = getenv("HOME")) != NULL && *p != '\0') {
			Com_sprintf(homePath, sizeof(homePath), "%s%c%s", p, PATH_SEP, HOMEPATH_NAME_UNIX_LEGACY);
		}
	}
...
static bool Sys_ShouldUseLegacyHomePath(void) {
	...
	Com_sprintf(migrationRefusedPath, sizeof(migrationRefusedPath), "%s/.xdgMigrationRefused", legacyHomePath);

	// If the user hasn't already refused, ask if they want to migrate
	if(access(migrationRefusedPath, F_OK) != 0) {
		dialogResult_t result = Sys_Dialog(DT_YES_NO,
		                                   va("Modern games and applications store files in "
		...
		if(result == DR_YES) return !Sys_MigrateToXDG();
...
char* Sys_DefaultHomeConfigPath(void) {
	if(Sys_ShouldUseLegacyHomePath()) return Sys_LegacyHomePath();
...
char* Sys_BinaryPathRelative(const char* relative) {
	...
	if(!realpath(combined, resolved)) return NULL;
