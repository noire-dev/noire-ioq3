...
void ClipWindingEpsilon(winding_t* in, vec3_t normal, vec_t dist, vec_t epsilon, winding_t** front, winding_t** back) {
	...
	*back = b = AllocWinding(maxpts);
	...
	if(f->numpoints > MAX_POINTS_ON_WINDING || b->numpoints > MAX_POINTS_ON_WINDING) Com_Error(ERR_DROP, "ClipWinding: MAX_POINTS_ON_WINDING");
}
...
void CheckWinding(winding_t* w) {
	...
	for(i = 0; i < w->numpoints; i++) {
		...
		for(j = 0; j < 3; j++)
			if(p1[j] > MAX_MAP_BOUNDS || p1[j] < -MAX_MAP_BOUNDS) Com_Error(ERR_DROP, "CheckFace: BUGUS_RANGE: %f", p1[j]);
