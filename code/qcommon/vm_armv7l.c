...
#define NOTIMPL(x)                                                          \
	do {                                                                    \
		Com_Error(ERR_DROP, "instruction not implemented: %s", opnames[x]); \
	} while(0)
#else
#define NOTIMPL(x)                                                      \
	do {                                                                \
...
static void Q_NO_RETURN ErrJump(unsigned num) { Com_Error(ERR_DROP, "program tried to execute code outside VM (%x)", num); }
...
#define STM(base, regs) (AL | 0b100 << 25 | 0 << 24 /*P*/ | 0 << 24 /*U*/ | 0 << 24 /*S*/ | 0 << 24 /*W*/ | (base << 16) | (regs & ~(1 << 16)))
