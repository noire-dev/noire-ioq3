...
ppc_instruction_t asm_instruction(powerpc_iname_t sname, const int argc, const long int* argv) {
	...
	while(opcode->operands[argi] != 0) {
		...
		if(operand->insert) {
			...
			if((bitm & 1) == 0) {
				if(opu & 0xf & ~bitm) printf("%s: operand nr.%d not aligned correctly. op: %.8lx, mask: %.8lx\n", name, argi, opu, bitm);
			}
 ...
 */
static inline void PPC_EmitNull(source_instruction_t* const i_null) { PPC_AppendInstructions(i_null->i_count, 0, NULL); }
...
static void VM_Destroy_Compiled(vm_t* self) {
	if(self->codeBase) {
		if(munmap(self->codeBase, self->codeLength)) Com_Printf(S_COLOR_RED "Memory unmap failed, possible memory leak\n");
	}
	self->codeBase = NULL;
}
