...
static duk_ret_t jsexport_cvar_float(duk_context* ctx) {
	...
	cvar_t* var = Cvar_FindVar(cvar_name);
	if(var)
		duk_push_number(ctx, (double)var->value);
	else
		duk_push_null(ctx);
...
static duk_ret_t jsexport_vmcall(duk_context* ctx) {
	...
	for(int i = 0; i < nargs - 2 && i < MAX_JS_ARGS; i++) {
		...
		if(duk_is_number(ctx, arg_idx)) {
			...
			if(val == (int)val)
				vmargs->t[i] = JS_TYPE_INT;
			else
				vmargs->t[i] = JS_TYPE_FLOAT;
	...
	if(args) {
		for(int i = 0; i < MAX_JS_ARGS; i++) {
			...
			switch(args->t[i]) {
				...
				case JS_TYPE_INT: duk_push_int(js_ctx, args->v[i].i); break;
				case JS_TYPE_FLOAT: duk_push_number(js_ctx, args->v[i].f); break;
				case JS_TYPE_STRING: duk_push_string(js_ctx, args->v[i].s); break;
			}
			arg_count++;
		}
	}
