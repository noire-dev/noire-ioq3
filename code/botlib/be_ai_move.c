...
int BotMovementViewTarget(int movestate, bot_goal_t* goal, int travelflags, float lookahead, vec3_t target) {
	...
	while(reachnum && dist < lookahead) {
		...
		if((reach.traveltype & TRAVELTYPE_MASK) != TRAVEL_JUMPPAD && (reach.traveltype & TRAVELTYPE_MASK) != TRAVEL_ELEVATOR &&
		...
		reachnum = BotGetReachabilityToGoal(reach.end,
		                                    reach.areanum,
		                                    ms->lastgoalareanum,
		                                    lastareanum,
		                                    ms->avoidreach,
...
int BotPredictVisiblePosition(vec3_t origin, int areanum, bot_goal_t* goal, int travelflags, vec3_t target) {
	...
	for(i = 0; i < 20 && (areanum != goal->areanum); i++) {
		//
		reachnum = BotGetReachabilityToGoal(end,
		                                    areanum,
		                                    lastgoalareanum,
		                                    lastareanum,
		                                    avoidreach,
	...
	if(ms->moveflags & (MFL_ONGROUND | MFL_SWIMMING | MFL_AGAINSTLADDER)) {
		...
		for(i = numareas - 1; i >= 0; i--) {
			if(AAS_AreaJumpPad(areas[i])) {
				...
				foundjumppad = qtrue;
				lastreachnum = BotGetReachabilityToGoal(end,
				                                        areas[i],
				                                        ms->lastgoalareanum,
				                                        ms->lastareanum,
				                                        ms->avoidreach,
