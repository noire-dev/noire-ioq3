...
int AAS_FloodClusterAreas_r(int areanum, int clusternum) {
	...
	if(!nofaceflood) {
		for(i = 0; i < area->numfaces; i++) {
			...
			if(face->frontarea == areanum) {
				if(face->backarea)
					if(!AAS_FloodClusterAreas_r(face->backarea, clusternum)) return false;
			}  // end if
			else {
				if(face->frontarea)
					if(!AAS_FloodClusterAreas_r(face->frontarea, clusternum)) return false;
			}  // end else
		}  // end for
	}  // end if
	...
	for(i = 0; i < aasworld.areasettings[areanum].numreachableareas; i++) {
		...
		if(!AAS_FloodClusterAreas_r(aasworld.reachability[aasworld.areasettings[areanum].firstreachablearea + i].areanum, clusternum)) return false;
	}  // end for
	return true;
	...
	for(i = 1; i < aasworld.numareas; i++) {
		...
		for(j = 0; j < aasworld.areasettings[i].numreachableareas; j++) {
			...
			if(aasworld.areasettings[areanum].cluster) {
				if(!AAS_FloodClusterAreas_r(i, clusternum)) return false;
				i = 0;
				break;
			}  // end if
		}  // end for
...
int AAS_FindClusters(void) {
	...
	for(i = 1; i < aasworld.numareas; i++) {
		...
		if(!AAS_FloodClusterAreas_r(i, aasworld.numclusters)) return false;
		if(!AAS_FloodClusterAreasUsingReachabilities(aasworld.numclusters)) return false;
