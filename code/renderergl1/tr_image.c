...
static void Upload32(unsigned* data,
	...
	*format = internalFormat;
	...
	if(mipmap) {
		...
		if(textureFilterAnisotropic) qglTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, 1);
	...
	if(scaledBuffer != 0) ri.Hunk_FreeTempMemory(scaledBuffer);
	if(resampledBuffer != 0) ri.Hunk_FreeTempMemory(resampledBuffer);
}
...
static imageExtToLoaderMap_t imageLoaders[] =
    {{"tga", R_LoadTGA}, {"jpg", R_LoadJPG}, {"jpeg", R_LoadJPG}, {"png", R_LoadPNG}, {"pcx", R_LoadPCX}, {"bmp", R_LoadBMP}, {"pvr", R_LoadPVR}};
