...
void ProjectDlightTexture_altivec(void) {
	...
	vector unsigned char vSel = VECCONST_UINT8(0x00, 0x00, 0x00, 0xff, 0x00, 0x00, 0x00, 0xff, 0x00, 0x00, 0x00, 0xff, 0x00, 0x00, 0x00, 0xff);
	float* texCoords;
	byte* colors;
	...
	for(l = 0; l < backEnd.refdef.num_dlights; l++) {
		...
		for(i = 0; i < tess.numVertexes; i++, texCoords += 2, colors += 4) {
			...
			if(!r_dlightBacks->integer &&
			   // dist . tess.normal[i]
			   (dist0 * tess.normal[i][0] + dist1 * tess.normal[i][1] + dist2 * tess.normal[i][2]) < 0.0f) {
				clip = 63;
			} else {
				if(texCoords0 < 0.0f) {
					clip |= 1;
