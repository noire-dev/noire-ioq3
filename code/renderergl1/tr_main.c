...
void R_TransformModelToClip(const vec3_t src, const float* modelMatrix, const float* projectionMatrix, vec4_t eye, vec4_t dst) {
	...
	for(i = 0; i < 4; i++) {
		dst[i] = eye[0] * projectionMatrix[i + 0 * 4] + eye[1] * projectionMatrix[i + 1 * 4] + eye[2] * projectionMatrix[i + 2 * 4] +
...
void R_SetupProjection(viewParms_t* dest, float zProj, qboolean computeFrustum) {
	...
	if(computeFrustum) R_SetupFrustum(dest, xmin, xmax, ymax, zProj, stereoSep);
}
...
void R_PlaneForSurface(surfaceType_t* surfType, cplane_t* plane) {
	...
	switch(*surfType) {
		case SF_FACE: *plane = ((srfSurfaceFace_t*)surfType)->plane; return;
		case SF_TRIANGLES:
