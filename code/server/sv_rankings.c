...
================
*/
bool SV_RankCheckInit(void) { return (s_rankings_contexts > 0); }
...
================
*/
bool SV_RankActive(void) { return s_rankings_active; }
...
================
*/
void SV_RankReportInt(int index1, int index2, int key, int value, bool accum) {
	GR_STATUS status;
	GR_CONTEXT context;
	uint64_t match;
	uint64_t user1;
...
static void SV_RankJoinGameCBF(GR_JOINGAME* gr_joingame, void* cbf_arg) {
	...
	if(gr_joingame->status == GR_STATUS_OK) {
		...
		for(i = 0; i < sv_maxclients->value; i++)
			if(ranked_player == &s_ranked_players[i]) SV_RankUserValidate(i, NULL, NULL, 0, gr_joingame->rank, ranked_player->name);
	} else {
