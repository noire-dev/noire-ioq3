...
static bool SV_IsBanned(netadr_t* from, bool isexception) {
	...
	if(!isexception) {
		...
		if(SV_IsBanned(from, true)) return false;
	}
	...
	for(index = 0; index < serverBansCount; index++) {
		...
		if(curban->isexception == isexception) {
			if(NET_CompareBaseAdrMask(curban->ip, *from, curban->subnet)) return true;
		}
	}
...
static void SV_StopDownload_f(client_t* cl) {
	if(*cl->downloadName) Com_DPrintf("clientDownload: %d : file \"%s\" aborted\n", (int)(cl - svs.clients), cl->downloadName);
...
int SV_WriteDownloadToClient(client_t* cl, msg_t* msg) {
	...
	if(!cl->download) {
		...
		if(!(sv_allowDownload->integer & DLF_ENABLE) || (sv_allowDownload->integer & DLF_NO_UDP) || idPack || unreferenced || (cl->downloadSize = FS_BaseDir_FOpenFileRead(cl->downloadName, &cl->download)) < 0) {
	...
	// Write the block
	if(cl->downloadBlockSize[curindex]) MSG_WriteData(msg, cl->downloadBlocks[curindex], cl->downloadBlockSize[curindex]);
