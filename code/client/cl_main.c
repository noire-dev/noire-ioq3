...
void CL_Record_f(void) {
	...
	if(Cmd_Argc() == 2) {
		...
		for(number = 0; number <= 9999; number++) {
			...
			if(!FS_FileExists_HomeData(name)) break;  // file doesn't exist
		}
	}
...
static void CL_CompleteDemoName(char* args, int argNum) {
	if(argNum == 2) {
		...
		Field_CompleteFilename("demos", "", demoFilter, false, true);
	}
}
...
void CL_Disconnect(bool showMainMenu) {
	...
	*clc.downloadTempName = *clc.downloadName = 0;
	...
	if(clc.voipCodecInitialized) {
		...
		clc.voipCodecInitialized = false;
	}
...
=================
*/
void CL_ResetPureClientAtServer(void) { CL_AddReliableCommand("vdr", false); }
...
void CL_DownloadsComplete(void) {
	...
	if(clc.httpUsed) {
		...
		if(clc.disconnectedForHttpDownload) {
			...
			clc.disconnectedForHttpDownload = false;
			CL_Reconnect_f();
			return;
		}
	}
...
=================
*/
static void CL_BeginDownload(const char* remoteName) { CL_AddReliableCommand(va("download %s", remoteName), false); }
...
static void CL_BeginHttpDownload(const char* remoteURL) {
	if(Q_strncmp(remoteURL, "http://", strlen("http://")) != 0 && Q_strncmp(remoteURL, "https://", strlen("https://")) != 0) {
		Com_Error(ERR_DROP,
	...
	if(!(clc.sv_allowDownload & DLF_NO_DISCONNECT) && !clc.disconnectedForHttpDownload) {
		...
		CL_WritePacket();
		CL_WritePacket();
		clc.disconnectedForHttpDownload = true;
	}
...
#ifndef STANDALONE
			if(!com_standalone->integer && clc.serverAddress.type == NA_IP && !Sys_IsLANAddress(clc.serverAddress)) CL_RequestAuthorization();
#endif
...
#ifdef LEGACY_PROTOCOL
			if(com_legacyprotocol->integer == com_protocol->integer) clc.compat = true;
...
#else
		Netchan_Setup(NS_CLIENT, &clc.netchan, from, Cvar_VariableValue("net_qport"), clc.challenge, false);
#endif
...
#ifdef USE_HTTP
	if(clc.httpUsed) {
		bool finished = CL_HTTP_PerformDownload();
...
void CL_Video_f(void) {
	...
	if(Cmd_Argc() == 2) {
		...
		for(i = 0; i <= 9999; i++) {
			...
			if(!FS_FileExists_HomeData(filename)) break;  // file doesn't exist
		}
...
void ParseOBJ(const char* objData, vec3_t* vertices, vec2_t* texCoords, vec3_t* normals, md3SurfaceData_t* surfaces, int* numSurfaces, int* vCount, int* vtCount, int* vnCount, const md3Material_t* materials, int numMaterials, md3Shader_t* shaders, int* numShaders) {
	...
	while(*p) {
		if(strncmp(p, "v ", 2) == 0 && *vCount < MD3_MAX_VERTS) {
			...
			int v[OBJ_MAX_FACE_VERTS], vt[OBJ_MAX_FACE_VERTS], vn[OBJ_MAX_FACE_VERTS];
			int count = 0;
			const char* faceStart = p + 2;

			// Считываем ВСЕ вершины грани, а не только первые 4
			while(*faceStart && count < OBJ_MAX_FACE_VERTS) {
				while(*faceStart == ' ') faceStart++;
				if(!*faceStart || *faceStart == '\r' || *faceStart == '\n') break;

				char* end = NULL;
				long val = strtol(faceStart, &end, 10);
				if(end == faceStart || val == 0) break;

				v[count] = (val > 0) ? ((int)val - 1) : (*vCount + (int)val);
				vt[count] = -1;
				vn[count] = -1;
				faceStart = end;
				...
				if(*faceStart == '/') {
					...
					if(*faceStart != '/') {
						...
						if(end != faceStart && val != 0) {
							vt[count] = (val > 0) ? ((int)val - 1) : (*vtCount + (int)val);
						}
						faceStart = end;
					}
					...
					if(*faceStart == '/') {
						...
						val = strtol(faceStart, &end, 10);
						if(end != faceStart && val != 0) {
							vn[count] = (val > 0) ? ((int)val - 1) : (*vnCount + (int)val);
						}
						faceStart = end;
					}
				}
			...
			if(count >= 3) {
				...
				if(valid) {
					int indices[OBJ_MAX_FACE_VERTS];
					for(int i = 0; i < count; i++) {
					...
					// Триангуляция N-угольника веером (Triangle Fan)
					if(valid) {
						for(int i = 1; i < count - 1; i++) {
							if(surfaces[currentSurface].triCount >= MD3_MAX_TRIANGLES) break;

							// Сохраняем порядок обхода вертушки (Winding Order)
							surfaces[currentSurface].triangles[surfaces[currentSurface].triCount].indexes[0] = indices[0];
							surfaces[currentSurface].triangles[surfaces[currentSurface].triCount].indexes[1] = indices[i + 1];
							surfaces[currentSurface].triangles[surfaces[currentSurface].triCount].indexes[2] = indices[i];
...
void WriteMD3(const char* name, const md3SurfaceData_t* surfaces, int numSurfaces, const md3Shader_t* shaders, int numShaders, const md3Material_t* materials, int numMaterials) {
	fileHandle_t f = FS_FOpenFileWrite_HomeData(va("%s.md3", name));
	if(!f) {
...
void CL_StartConvertOBJ(const char* name) {
	...
	ParseOBJ(objData, vertices, texCoords, normals, surfaces, &numSurfaces, &vCount, &vtCount, &vnCount, materials, numMaterials, shaders, &numShaders);
...
void CL_Sayto_f(void) {
	...
	CL_AddReliableCommand(va("tell %i \"%s\"", clientNum, p), false);
}
