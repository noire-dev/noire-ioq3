...
void S_TransferStereo16(unsigned long* pbuf, int endtime) {
	...
	while(ls_paintedtime < endtime) {
		...
		if(CL_VideoRecording()) CL_WriteAVIAudioFrame((byte*)snd_out, snd_linear_count << 1);  // snd_linear_count * (dma.samplebits/8)
	}
}
...
void S_TransferPaintBuffer(int endtime) {
	...
	if(s_testsound->integer) {
		...
		for(i = 0; i < count; i++) paintbuffer[i].left = paintbuffer[i].right = sin((s_paintedtime + i) * 0.1) * 20000 * 256;
	}
