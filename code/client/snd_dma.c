...
====================
*/
void S_Base_StartSound(vec3_t origin, int entityNum, int entchannel, sfxHandle_t sfxHandle) { S_Base_StartSoundEx(origin, entityNum, entchannel, sfxHandle, false); }
...
void S_Base_ClearSoundBuffer(void) {
	...
	if(dma.buffer) Com_Memset(dma.buffer, clear, dma.samples * dma.samplebits / 8);
	SNDDMA_Submit();
}
