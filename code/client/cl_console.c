...
void Con_Linefeed(qboolean skipnotify) {
	...
	con.current++;
	for(i = 0; i < con.linewidth; i++) con.text[(con.current % con.totallines) * con.linewidth + i] = (ColorIndex(COLOR_WHITE) << 8) | ' ';
}
...
void Con_DrawInput(void) {
	...
	Field_Draw(&g_consoleField, con.xadjust + 2 * g_smallchar_width, y, SCREEN_WIDTH - 3 * g_smallchar_width, qtrue, qtrue);
}
...
void Con_DrawSolidConsole(float frac) {
	...
	if(con.display != con.current) {
		...
		re.SetColor(g_color_table[ColorIndex(COLOR_RED)]);
		for(x = 0; x < con.linewidth; x += 4) SCR_DrawSmallChar(con.xadjust + (x + 1) * g_smallchar_width, y, '^');
