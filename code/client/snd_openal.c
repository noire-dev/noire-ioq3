...
static void S_AL_BufferUnload(sfxHandle_t sfx) {
	...
	if(qalGetError() != AL_NO_ERROR) Com_Printf(S_COLOR_RED "ERROR: Can't delete sound buffer for %s\n", knownSfx[sfx].filename);
...
static void S_AL_BufferUse(sfxHandle_t sfx) {
	...
	if((!knownSfx[sfx].inMemory) && (!knownSfx[sfx].isDefault)) S_AL_BufferLoad(sfx, true);
...
static bool S_AL_BufferInit(void) {
	if(alBuffersInitialised) return true;
...
static void S_AL_SrcShutdown(void) {
	...
	for(i = 0; i < srcCount; i++) {
		...
		if(curSource->entity > 0) entityList[curSource->entity].srcAllocated = false;
...
=================
*/
static void S_AL_SrcLock(srcHandle_t src) { srcList[src].isLocked = true; }
...
=================
*/
static void S_AL_SrcUnlock(srcHandle_t src) { srcList[src].isLocked = false; }
...
static void S_AL_UpdateEntityPosition(int entityNum, const vec3_t origin) {
	...
	if(entityNum < 0 || entityNum >= MAX_GENTITIES) Com_Error(ERR_DROP, "S_UpdateEntityPosition: bad entitynum %i", entityNum);
...
static bool S_AL_CheckInput(int entityNum, sfxHandle_t sfx) {
	if(entityNum < 0 || entityNum >= MAX_GENTITIES) Com_Error(ERR_DROP, "ERROR: S_AL_CheckInput: bad entitynum %i", entityNum);
...
static void S_AL_StartSound(vec3_t origin, int entnum, int entchannel, sfxHandle_t sfx) {
	...
	if(!origin) curSource->isTracking = true;
...
static void S_AL_ClearLoopingSounds(bool killall) {
	int i;
	for(i = 0; i < srcCount; i++) {
		if((srcList[i].isLooping) && (srcList[i].entity != -1)) entityList[srcList[i].entity].loopAddedThisFrame = false;
	}
}
...
=================
*/
static void S_AL_AddLoopingSound(int entityNum, const vec3_t origin, const vec3_t velocity, sfxHandle_t sfx) { S_AL_SrcLoop(SRCPRI_ENTITY, sfx, origin, velocity, entityNum); }
...
=================
*/
static void S_AL_AddRealLoopingSound(int entityNum, const vec3_t origin, const vec3_t velocity, sfxHandle_t sfx) { S_AL_SrcLoop(SRCPRI_AMBIENT, sfx, origin, velocity, entityNum); }
...
static void S_AL_SrcUpdate(void) {
	...
	for(i = 0; i < srcCount; i++) {
		...
		if((s_alRolloff->modified) && (!curSource->local)) qalSourcef(curSource->alSource, AL_ROLLOFF_FACTOR, s_alRolloff->value);
...
static void S_AL_RawSamples(int stream, int samples, int rate, int width, int channels, const byte* data, float volume, int entityNum) {
	...
	if(numBuffers == streamNumBuffers[stream]) {
		...
		for(i = 0; i < streamNumBuffers[stream]; ++i) streamBuffers[stream][i] = oldBuffers[(streamBufIndex[stream] + i) % streamNumBuffers[stream]];
...
static void S_AL_SoundList(void) {
	...
	for(sfx = knownSfx, i = 0; i < numSfx; i++, sfx++) {
		...
		total += size;
		Com_Printf("%6i : %s[%s]\n", size, sfx->filename, sfx->inMemory ? "resident " : "paged out");
	}
