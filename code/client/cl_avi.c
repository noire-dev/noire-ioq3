...
static ID_INLINE void SafeFS_Write(const void* buf, int len, fileHandle_t f) {
	if(FS_Write(buf, len, f) < len) Com_Error(ERR_DROP, "Failed to write avi file");
}
...
bool CL_OpenAVIForWriting(const char* fileName) {
	...
	if((afd.f = FS_FOpenFileWrite_HomeData(fileName)) <= 0) return false;

	if((afd.idxF = FS_FOpenFileWrite_HomeData(va("%s" INDEX_FILE_EXTENSION, fileName))) <= 0) {
		FS_FCloseFile(afd.f);
		return false;
	}
...
===============
*/
bool CL_VideoRecording(void) { return afd.fileOpen; }
