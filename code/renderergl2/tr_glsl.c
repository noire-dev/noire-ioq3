...
static int GLSL_InitGPUShader(shaderProgram_t* program,
                              const char* name,
                              int attribs,
                              qboolean fragmentShader,
                              const GLchar* extra,
                              qboolean addHeader,
                              const char* fallback_vp,
...
void GLSL_InitGPUShaders(void) {
	...
	for(i = 0; i < GENERICDEF_COUNT; i++) {
		...
		if(i & GENERICDEF_USE_VERTEX_ANIMATION) {
			if(!glRefConfig.gpuVertexAnimation) continue;
	...
	if(!GLSL_InitGPUShader(&tr.textureColorShader,
	                       "texturecolor",
	                       attribs,
	                       qtrue,
	                       extradefines,
	                       qtrue,
	                       fallbackShader_texturecolor_vp,
	...
	for(i = 0; i < FOGDEF_COUNT; i++) {
		...
		if((i & FOGDEF_USE_VERTEX_ANIMATION) && !glRefConfig.gpuVertexAnimation) continue;
	...
	for(i = 0; i < LIGHTDEF_COUNT; i++) {
		...
		if(lightType) {
			...
			if(r_normalMapping->integer) {
				...
				if((i & LIGHTDEF_USE_PARALLAXMAP) && !(i & LIGHTDEF_ENTITY_VERTEX_ANIMATION) && !(i & LIGHTDEF_ENTITY_BONE_ANIMATION) &&
		...
		if(!GLSL_InitGPUShader(&tr.lightallShader[i],
		                       "lightall",
		                       attribs,
		                       qtrue,
		                       extradefines,
		                       qtrue,
		                       fallbackShader_lightall_vp,
	...
	for(i = 0; i < SHADOWMAPDEF_COUNT; i++) {
		...
		if((i & SHADOWMAPDEF_USE_VERTEX_ANIMATION) && !glRefConfig.gpuVertexAnimation) continue;
		...
		if(!GLSL_InitGPUShader(&tr.shadowmapShader[i],
		                       "shadowfill",
		                       attribs,
		                       qtrue,
		                       extradefines,
		                       qtrue,
		                       fallbackShader_shadowfill_vp,
		                       fallbackShader_shadowfill_fp)) {
	...
	for(i = 0; i < 2; i++) {
		...
		if(!GLSL_InitGPUShader(&tr.calclevels4xShader[i],
		                       "calclevels4x",
		                       attribs,
		                       qtrue,
		                       extradefines,
		                       qtrue,
		                       fallbackShader_calclevels4x_vp,
		                       fallbackShader_calclevels4x_fp)) {
	...
	if(glRefConfig.glslMajorVersion > 1 || (glRefConfig.glslMajorVersion == 1 && glRefConfig.glslMinorVersion >= 10) || glRefConfig.shadowSamplers) {
		attribs = ATTR_POSITION | ATTR_TEXCOORD;
		extradefines[0] = '\0';
		...
		if(!GLSL_InitGPUShader(&tr.shadowmaskShader,
		                       ...
		                       extradefines,
		                       qtrue,
		                       fallbackShader_shadowmask_vp,
		                       fallbackShader_shadowmask_fp)) {
	...
	if(glRefConfig.glslMajorVersion > 1 || (glRefConfig.glslMajorVersion == 1 && glRefConfig.glslMinorVersion >= 10) || glRefConfig.standardDerivatives) {
		attribs = ATTR_POSITION | ATTR_TEXCOORD;
		extradefines[0] = '\0';
		...
		for(i = 0; i < 4; i++) {
			...
			if(!GLSL_InitGPUShader(&tr.depthBlurShader[i],
			                       "depthBlur",
			                       attribs,
			                       qtrue,
			                       extradefines,
			                       qtrue,
			                       fallbackShader_depthblur_vp,
			                       fallbackShader_depthblur_fp)) {
	...
	ri.Printf(PRINT_ALL,
	          ...
	          numGenShaders + numLightShaders + numEtcShaders,
	          numGenShaders,
	          numLightShaders,
	          numEtcShaders,
...
void GLSL_ShutdownGPUShaders(void) {
	...
	for(i = 0; i < ATTR_INDEX_COUNT && i < glRefConfig.maxVertexAttribs; i++) qglDisableVertexAttribArray(i);
