...
static void CollapseStagesToLightall(shaderStage_t* diffuse, shaderStage_t* normal, shaderStage_t* specular, shaderStage_t* lightmap, bool useLightVector, bool useLightVertex, bool parallax, bool tcgen) {
	int defs = 0;
...
static int CollapseStagesToGLSL(void) {
	...
	if(!skip) {
		...
		for(i = 0; i < MAX_SHADER_STAGES; i++) {
			...
			switch(pStage->alphaGen) {
				...
				case AGEN_PORTAL: skip = true; break;
				default: break;
			}
		}
	}
	...
	if(r_sunlightMode->integer && shader.numDeforms == 0) {
		for(i = 0; i < MAX_SHADER_STAGES; i++) {
			...
			if(pStage->bundle[TB_DIFFUSEMAP].tcGen != TCGEN_LIGHTMAP) continue;

			blendBits = pStage->stateBits & (GLS_DSTBLEND_BITS | GLS_SRCBLEND_BITS);

			if(blendBits != 0 && blendBits != (GLS_DSTBLEND_SRC_COLOR | GLS_SRCBLEND_ZERO) && blendBits != (GLS_DSTBLEND_ZERO | GLS_SRCBLEND_DST_COLOR)) {
				continue;
			}
	...
	if(shader.numDeforms == 0) {
		for(i = 0; i < MAX_SHADER_STAGES; i++) {
			...
			if(pStage->rgbGen == CGEN_LIGHTING_DIFFUSE) {
				...
				if(pStage->bundle[0].tcGen != TCGEN_TEXTURE || pStage->bundle[0].numTexMods != 0) pStage->glslShaderIndex |= LIGHTDEF_USE_TCGEN_AND_TCMOD;
			}
		}
	}
...
static void InitShader(const char* name, int lightmapIndex) { InitShaderEx(name, lightmapIndex, lightmapIndex); }
...
===============
*/
shader_t* R_FindShader(const char* name, int lightmapIndex, bool mipRawImage) { return R_FindShaderEx(name, lightmapIndex, mipRawImage, lightmapIndex); }
