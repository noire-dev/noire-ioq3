...
void Vao_SetVertexPointers(vao_t* vao) {
	...
	for(attribIndex = 0; attribIndex < ATTR_INDEX_COUNT; attribIndex++) {
		...
		if(vAtb->enabled) {
			...
			if(glRefConfig.vertexArrayObject || !(glState.vertexAttribsEnabled & attribBit)) qglEnableVertexAttribArray(attribIndex);
			...
			if(!glRefConfig.vertexArrayObject && (glState.vertexAttribsEnabled & attribBit)) qglDisableVertexAttribArray(attribIndex);
	...
	for(; batchLength < vc.batchLengths + vc.numBatches; batchLength++) {
		if(*batchLength == vcq.numSurfaces) {
			...
			for(surf = vcq.surfaces; surf < end; surf++, indexSet2++) {
				if(surf->indexes != indexSet2->indexes || surf->numIndexes != indexSet2->numIndexes) break;
			}
		...
		*batchLength = vcq.numSurfaces;
		...
		for(surf = vcq.surfaces; surf < end; surf++) {
			...
			if(glRefConfig.vaoCacheGlIndexType == GL_UNSIGNED_SHORT) {
				for(i = 0; i < surf->numIndexes; i++) *dstIndexUshort++ = *srcIndex++ + indexOffset;
			} else {
		...
		// ri.Printf(PRINT_ALL, "committing %d to %d, %d to %d as %d\n", vcq.vertexCommitSize, vc.vertexOffset, vcq.indexCommitSize, vc.indexOffset,
		// (int)(batchLength - vc.batchLengths));
