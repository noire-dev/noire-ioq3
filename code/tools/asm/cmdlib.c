...
char* ExpandPath(const char* path) {
	static char full[1024];
	if(!qdir[0]) Error("ExpandPath called without qdir set");
	if(path[0] == '/' || path[0] == '\\' || path[1] == ':') {
		strcpy(full, path);
		return full;
	}
...
char* ExpandGamePath(const char* path) {
	static char full[1024];
	if(!qdir[0]) Error("ExpandGamePath called without qdir set");
	if(path[0] == '/' || path[0] == '\\' || path[1] == ':') {
		strcpy(full, path);
		return full;
	}
...
#ifdef WIN32
	if(_getcwd(out, 256) == NULL) strcpy(out, "."); /* shrug */
	strcat(out, "\\");
#else
	if(getcwd(out, 256) == NULL) strcpy(out, "."); /* shrug */
	strcat(out, "/");
#endif
...
int CheckParm(const char* check) {
	...
	for(i = 1; i < myargc; i++) {
		if(!Q_stricmp(check, myargv[i])) return i;
	}
...
void SafeWrite(FILE* f, const void* buffer, int count) {
	if(fwrite(buffer, 1, count, f) != (size_t)count) Error("File write failure");
}
...
void StripExtension(char* path) {
	...
	while(length > 0 && path[length] != '.') {
		length--;
		if(path[length] == '/') return;  // no extension
	}
