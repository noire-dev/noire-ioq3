...
static int vreport(const char* fmt, va_list vp) {
	if(options.verbose != true) return 0;
...
static int ParseExpression(void) {
	...
	switch(*sym) { /* Resolve depending on first character. */
		...
		case '7':
		case '8':
		case '9': v = atoiNoCap(sym); break;
		default: v = LookupSymbol(sym); break;
	}
...
int main(int argc, char** argv) {
	...
	for(i = 1; i < argc; i++) {
		...
		if(!strcmp(argv[i], "-m")) {
			options.writeMapFile = true;
			continue;
		}

		if(!strcmp(argv[i], "-vq3")) {
			options.vanillaQ3Compatibility = true;
			continue;
		}
